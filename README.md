...
```
docker run -it kave/kavetoolbox:3.2-Beta.c7.node /bin/bash
```
...
* 1: Installing the released version, for example, 3.2-Beta

```
yum -y install wget curl tar zip unzip gzip python
wget http://repos:kaverepos@repos.kave.io/noarch/KaveToolbox/3.2-Beta/kavetoolbox-installer-3.2-Beta.sh
sudo bash kavetoolbox-installer-3.2-Beta.sh [--quiet]
```
