...
```
docker run -it kave/kavetoolbox:3.2-Beta-Pre.c7.node /bin/bash
```
...
* 1: Installing the released version, for example, 3.2-Beta-Pre

```
yum -y install wget curl tar zip unzip gzip python
wget http://repos:kaverepos@repos.kave.io/noarch/KaveToolbox/3.2-Beta-Pre/kavetoolbox-installer-3.2-Beta-Pre.sh
sudo bash kavetoolbox-installer-3.2-Beta-Pre.sh [--quiet]
```
